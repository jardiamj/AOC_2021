...
class Compare {
public:
    bool operator()(const shared_ptr<Cell> a, const shared_ptr<Cell> b) {
        return a->risk > b->risk;
    }
};

void print_risk_map(const vector<vector<int>>& risk_map);
void print_policy(const vector<vector<shared_ptr<Cell>>>& policy);
void find_shortest_path(const vector<vector<int>>& risk_map,
                        vector<vector<shared_ptr<Cell>>>& policy);
bool is_valid(int row, int col, int max_row, int max_col);
void print_actions(const vector<vector<shared_ptr<Cell>>>& policy);
void print_risk(const vector<vector<shared_ptr<Cell>>>& policy);
...
int main(int argc, char* argv[]) {
    ...
    for (int row = 0; row < risk_map.size(); row++) {
        vector<shared_ptr<Cell>> cell_row;
        for (int col = 0; col < risk_map[0].size(); col++) {
            shared_ptr<Cell> new_cell(new Cell);
            new_cell->row = row;
            new_cell->col = col;
                        ...
                        vector<vector<shared_ptr<Cell>>>& policy)
{
    ...
    priority_queue<shared_ptr<Cell>, vector<shared_ptr<Cell>>, Compare> open;
    policy[0][0]->risk = 0;
